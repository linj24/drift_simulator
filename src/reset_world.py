...
class ResetWorld(object):
    ...
    def generate_random_corner(self, angle_min: float = -np.pi, angle_max: float = np.pi) -> None:
        ...
        n_o = quaternion_from_euler(0, 0, angle / 2 + np.pi/2)

        motionless_twist = Twist(linear=Vector3(0,0,0), angular=Vector3(0,0,0))

        m_s1 = ModelState(model_name=self.wall_model_names["s1"], pose=Pose(position=Point(s1_x, s1_y, 0), orientation=Quaternion(*s1_o)), twist=motionless_twist)
        m_s2 = ModelState(model_name=self.wall_model_names["s2"], pose=Pose(position=Point(s2_x, s2_y, 0), orientation=Quaternion(*s2_o)), twist=motionless_twist)
        m_l1 = ModelState(model_name=self.wall_model_names["l1"], pose=Pose(position=Point(l1_x, l1_y, 0), orientation=Quaternion(*l1_o)), twist=motionless_twist)
        m_l2 = ModelState(model_name=self.wall_model_names["l2"], pose=Pose(position=Point(l2_x, l2_y, 0), orientation=Quaternion(*l2_o)), twist=motionless_twist)
        ...
        if np.abs(angle) < np.pi / 2:
            m_n = ModelState(model_name=self.wall_model_names["n"], pose=Pose(position=Point(n_x, n_y, 0), orientation=Quaternion(*n_o)), twist=motionless_twist)
        else:
            m_n = ModelState(model_name=self.wall_model_names["n"], pose=Pose(position=Point(15, 0, 0), orientation=Quaternion(x=0, y=0, z=0, w=1)), twist=motionless_twist)
        self.model_states_pub.publish(m_n)
